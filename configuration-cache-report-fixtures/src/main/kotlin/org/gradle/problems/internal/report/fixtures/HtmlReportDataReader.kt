...
class HtmlReportDataReader(private val reportFile: File) {
    ...
    fun findElement(elementId: String): String? =
        reportFile.useLines { lines ->
            readElementFrom(elementId, lines)
        }
    ...
    companion object {
         ...
         */
        fun readElementFrom(elementId: String, lines: Sequence<String>): String? {
            val openingTag = ReportDataElement.openingTag(elementId)
            val contents = lines.dropWhile { it != openingTag }
                .drop(1)
