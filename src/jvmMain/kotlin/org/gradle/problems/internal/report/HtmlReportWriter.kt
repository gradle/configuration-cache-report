...
class HtmlReportWriter internal constructor(
    ...
    fun beginHtmlReport() {
        ...
        writer.run {
            appendLine(openingTagOf(DIAGNOSTICS_ELEMENT_ID))
            appendLine("[")
        }
    }
    ...
    fun endHtmlReport(summary: JsReportSummary) {
        writer.run {
            ...
            appendLine("]")
            appendLine("</script>")
            appendLine(openingTagOf(summary.elementId))
    ...
    private
    fun openingTagOf(elementId: String) = """<script type="application/json" id="$elementId">"""
}
