...
/**
 * Loads the report HTML template bundled with this library and splits it at the placeholder
 * `<script>` element that stands in for the report data, so that [HtmlReportWriter] can emit the
 * data in its place.
 */
internal
class HtmlReportTemplateLoader {
    ...
    private
    val reportHtmlFileName = "configuration-cache-report.html"

    private
    fun readHtmlTemplate() =
        HtmlReportTemplateLoader::class.java
